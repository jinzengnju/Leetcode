...
//
//         动态规划的思想，考虑{arr[0],...,arr[i]}的最长递增子序列时需要找到所有比arr[i]小的arr[j]，且j<i，结果应该是所有{arr[0],...,arr[j]}的最长递增子序列中
        ...
        for(int i=0;i<n;i++){
            ...
            for(int j=0;j<i;j++){
                if(arr[j]<arr[i]){
                    ...
                    //max为j取当前值时的最大LIS长度,这里有个误区就是你可能认为max肯定是比dp[j]+1小的,其实不是
                    //比如现在max记录的是前一步j-1时的max,到新的j时候需要比较max与dp[j]+1值
                    //比如12,13,15,5,20,当i指向20的时候,j指向5的时候,max比dp[j]+1大
                }
            }
            dp[i]=max;
        }
